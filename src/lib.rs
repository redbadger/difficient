...
//!     x: "hello",
//!     y: (),
//!     z: SimpleStruct { a: "aaa".into(), b: 123, c: vec![1.23] }
//! };
//!
//! let diff2 = source.diff(&target1);
//! let expect = DeepDiff::Replaced(&target1);
...
//!     x: "goodbye",
//!     y: (),
//!     z: SimpleStruct { a: "aaa".into(), b: 234, c: vec![1.23] }
//! };
//!
...
//!     a: String,
//!     b: i32,
//!     #[diffable(skip)]  // do not take `c` into account when diffing
//!     c: Vec<f64>,
//! }
//!
...
//! source.apply(&diff).unwrap();
//! assert_eq!(source.a, target.a);
//! assert_eq!(source.b, target.b);
//! // SimpleStruct.c is unchanged, since we annotated with #[diffable(skip)]
//! assert!(source.c != target.c);
//! ```
